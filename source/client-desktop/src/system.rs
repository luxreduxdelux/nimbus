...
impl System {
    ...
    pub fn new(user: &User, ui: &egui::Context) -> Self {
        ...
        std::thread::spawn(move || {
            ...
            let icon_notify_a = Icon::from_rgba(icon_notify_a.to_rgba8().to_vec(), 64, 64).unwrap();
            let icon_notify_b = Icon::from_rgba(icon_notify_b.to_rgba8().to_vec(), 64, 64).unwrap();
            ...
            tray_icon::menu::MenuEvent::set_event_handler(Some(
                move |event: MenuEvent| match event.id.0.as_str() {
                    "1" => {
